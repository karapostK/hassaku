...
class PrototypeAdd(PrototypePerturb):
    ...
    def __init__(self, n_prototypes: int, n_groups: int = 1, optimize_parameters: bool = True,
        ...
        if default_parameters is None:
            self.deltas = nn.Parameter(0.1 * torch.randn(self.n_groups, self.n_prototypes))
        else:
...
class PrototypeTuner(SGDBasedRecommenderAlgorithm):
    ...
    user indexes to group indexes. Each group, then gets different tuning parameters.
    ...
    def forward(self, u_idxs: torch.Tensor, i_idxs: torch.Tensor) -> torch.Tensor:
        ...
        u_repr = self.get_user_representations(i_repr)
    ...
    def combine_user_item_representations(self, u_repr: Union[torch.Tensor, Tuple[torch.Tensor, ...]],
        ...
        if self.entity_name == 'item':
            ...
            # NB. This will create an item representation that is user-dependent.
            # So the effective shape will be [n_items, batch_users, n_prototypes] instead of [n_items, n_prototypes]
            i_repr_new = self.prototype_perturb(i_middle_repr.unsqueeze(1), group_idxs)
            i_repr_new = i_repr_new.transpose_(0, 1)
            if type(i_repr) == tuple:
